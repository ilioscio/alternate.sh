...
| NixOS VM integration test | `nix/tests.nix` | `nix build .#checks.x86_64-linux.commands` | Every shell command, end to end, over real SSH with a real PostgreSQL — including the arcade: lobby, wumpus, a full chess game (challenge→accept→moves→resign→leaderboard), and trade (universe gen, deterministic port, purchase guards) |

...
| NixOS VM integration test | `nix/tests.nix` | `nix build .#checks.x86_64-linux.commands` | Every shell command, end to end, over real SSH with a real PostgreSQL — including the arcade (lobby, wumpus, full chess game, trade) and community polish: fortune submit/review, mailing lists (fan-out, announce-only, namespace guard), moderation queue (pending invisible→approve/reject), ban/unban on both login surfaces, audit log |
...
| Go unit tests | `internal/games/chess/*_test.go` | `go test ./internal/games/chess/` | Bespoke chess engine: perft against 13 published node counts (~12M nodes — the standard correctness oracle), mate/stalemate/draws, en passant, castling rights, pins, SAN |
| Go unit tests | `internal/games/wumpus/`, `internal/games/trade/`, `internal/db/trade_test.go` | `go test ./internal/games/... ./internal/db/` | Wumpus cave topology + hazard/arrow mechanics (hand-placed states); trade price curve + commodity parsing; warp-graph connectivity/symmetry across seeds |
